...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Tambah Lapak Baru</h1>
            <p class="text-gray-400 text-sm mt-1">Isi data lapak Anda untuk mulai berjualan</p>
        </div>
    </div>
    ...
    {{-- Form --}}
    <form action="{{ route('vendor.vendors.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-[#1a1a2e] border border-gray-700/50 rounded-xl p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  ...
                                  placeholder="Ceritakan tentang lapak Anda...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <div id="image-preview-container" class="hidden mb-3">
                                <img id="image-preview" src="" alt="Preview" class="w-full h-48 object-cover rounded-lg border border-gray-700">
                                <button type="button" onclick="clearImage()" class="absolute top-2 right-2 p-1 rounded-full bg-red-500/80 text-white hover:bg-red-500 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
                                </button>
                            </div>
                            <label for="image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-700 rounded-lg cursor-pointer hover:border-orange-500/50 transition-colors" id="image-drop-zone">
                ...
                <div class="bg-[#1a1a2e] border border-gray-700/50 rounded-xl p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="time" name="open_time" id="open_time" value="{{ old('open_time') }}"
                                   class="w-full bg-[#0f0f23] border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors">
                            @error('open_time')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="time" name="close_time" id="close_time" value="{{ old('close_time') }}"
                                   class="w-full bg-[#0f0f23] border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors">
                            @error('close_time')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
            ...
            <div class="space-y-6">
                <div class="bg-[#1a1a2e] border border-gray-700/50 rounded-xl p-6 space-y-5">
                    ...
                    {{-- Map --}}
                    <div>
                        <p class="text-sm text-gray-400 mb-2">Klik pada peta untuk menandai lokasi lapak Anda</p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-[#0f0f23] rounded-lg px-3 py-2 border border-gray-700/50">
                            ...
                            <span id="lat-display" class="text-sm text-gray-300 font-mono">{{ old('latitude', '-6.2088') }}</span>
                        </div>
                        <div class="bg-[#0f0f23] rounded-lg px-3 py-2 border border-gray-700/50">
                            <span class="text-xs text-gray-500 block">Longitude</span>
                            <span id="lng-display" class="text-sm text-gray-300 font-mono">{{ old('longitude', '106.8456') }}</span>
                        </div>
                    </div>
